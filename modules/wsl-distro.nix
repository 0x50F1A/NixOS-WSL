...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    environment = {
      ...
      etc = mkMerge [
        (mkIf config.wsl.wslConf.network.generateHosts { hosts.enable = false; })
        (mkIf config.wsl.wslConf.network.generateResolvConf { "resolv.conf".enable = false; })
      ];
    };
    ...
    system.activationScripts = {
      ...
      populateBin = lib.mkIf cfg.populateBin (
        stringAfter [ ] ''
          echo "setting up /bin..."
          ${concatStringsSep "\n" (
            map (
              entry:
              if entry.copy then
                "cp -f ${entry.src} /bin/${entry.name}"
              else
                "ln -sf ${entry.src} /bin/${entry.name}"
            ) config.wsl.extraBin
          )}
        ''
      );
    ...
    warnings = flatten [
      ...
      (optional
        ((length config.networking.nameservers) > 0 && config.wsl.wslConf.network.generateResolvConf)
      ...
      (optional
        (
          (length config.networking.nameservers) == 0
          && !config.services.resolved.enable
          && !config.wsl.wslConf.network.generateResolvConf
        )
